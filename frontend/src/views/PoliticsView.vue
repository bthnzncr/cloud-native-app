<template>
  <div class="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    <h2 class="text-2xl font-bold mb-4 text-gray-200 mt-12">Politics News</h2>
    ...
    <div v-if="newsStore.newsItems.length === 0 && !newsStore.loading" class="text-center text-gray-500 py-4">
      No politics news articles found
    </div>
...
const loadNews = async () => {
  await newsStore.fetchNewsByCategory('POLITICS');
}
  ...
  scrollThrottleTimeout.value = setTimeout(() => {
    ...
    if (scrollPosition >= scrollThreshold) {
      if (!newsStore.loading && !newsStore.loadingMore && newsStore.hasMore) {
        console.log('Near bottom of page, loading more politics articles silently')
        loadMoreSilently()
      }
    }
