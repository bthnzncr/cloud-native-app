<template>
  <div class="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    <h2 class="text-2xl font-bold mb-4 text-gray-200 mt-12">
      Search Results: "{{ $route.query.q }}"
    </h2>
    ...
    <div v-if="newsStore.loading" class="text-center py-4">
      ...
      <p class="mt-2 text-gray-400">Searching news...</p>
    </div>
    
    <!-- No results message -->
    <div v-if="!newsStore.loading && newsStore.newsItems.length === 0" class="text-center py-8">
      <p class="text-gray-400 text-lg">No results found for "{{ $route.query.q }}"</p>
      <p class="text-gray-500 mt-2">Try different keywords or browse categories from the sidebar.</p>
    </div>
...
const handleScroll = () => {
  ...
  scrollThrottleTimeout.value = setTimeout(() => {
    ...
    const scrollThreshold = pageHeight - 300
    
    if (scrollPosition >= scrollThreshold) {
      if (!newsStore.loading && !newsStore.loadingMore && newsStore.hasMore) {
        console.log('Near bottom of page, loading more search results silently')
        loadMoreSilently()
      }
    }
...
onMounted(() => {
  // Load search results immediately if there's a query
  if (searchQuery.value) {
    loadSearchResults()
  }
