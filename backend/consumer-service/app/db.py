...
async def _ensure_indexes(db: AsyncIOMotorDatabase):
    ...
    try:
        ...
        # And an index on published_date for date-based queries
        await db[settings.ARTICLE_COLLECTION].create_index([("link", ASCENDING)], unique=True)
        await db[settings.ARTICLE_COLLECTION].create_index([("published_date", ASCENDING)])
        
        # Combined index for deduplication queries
        await db[settings.ARTICLE_COLLECTION].create_index([
            ("source", ASCENDING), 
